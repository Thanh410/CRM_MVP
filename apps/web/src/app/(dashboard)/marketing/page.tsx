      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg">
        <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-100">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-zinc-900">Chi tiết chiến dịch</h2>
          </div>
        ...
        {isLoading ? (
          ...
          <div className="px-6 py-5 space-y-5">
            ...
            {data.logs && Object.keys(data.logs).length > 0 && (
              <div>
                <p className="text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2">Chi tiết log</p>
  ...
  return (
    <div className="group bg-white rounded-xl border border-zinc-200 p-5 hover:shadow-md transition-shadow relative">
      ...
      <div className="absolute top-3 right-3 hidden group-hover:flex gap-1">
        <button onClick={() => onEdit(campaign)} className="p-1.5 text-zinc-400 hover:text-zinc-900 hover:bg-zinc-100 rounded-lg" title="Chỉnh sửa">
          <Pencil size={13} />
        </button>
        <button onClick={() => onDelete(campaign)} className="p-1.5 text-zinc-400 hover:text-red-600 hover:bg-red-50 rounded-lg" title="Xóa">
          <Trash2 size={13} />
        </button>
      </div>
      ...
      {campaign.startDate && (
        <p className="text-xs text-zinc-400 mb-3">📅 {formatDate(campaign.startDate)}</p>
      )}
    ...
    <div ref={overlayRef} className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4"
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
        ...
        <form onSubmit={e => { e.preventDefault(); update.mutate(form); }} className="px-6 py-5 space-y-4">
          <div>
            <label className="block text-xs font-medium text-zinc-600 mb-1">Tên chiến dịch *</label>
            <input className={inputCls} value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} required />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-zinc-600 mb-1">Kênh</label>
              <select className={inputCls} value={form.channel} onChange={e => setForm(p => ({ ...p, channel: e.target.value }))}>
                {Object.entries(CHANNEL_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-medium text-zinc-600 mb-1">Ngày bắt đầu</label>
          ...
          <div className="flex gap-2 pt-2 border-t border-zinc-100">
            <button type="button" onClick={onClose} className="flex-1 border border-gray-300 rounded-lg py-2 text-sm hover:bg-zinc-50">Hủy</button>
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={e => { e.preventDefault(); create.mutate(form); }} className="px-6 py-5 space-y-4">
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-600 mb-1">Template (tuỳ chọn)</label>
            <select className={inputCls} value={form.templateId} onChange={e => setForm(p => ({ ...p, templateId: e.target.value }))}>
              <option value="">-- Chọn template --</option>
              {templates.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
            </select>
          ...
          <div className="border border-zinc-200 rounded-xl p-4 space-y-3 bg-zinc-50/50">
            <p className="text-xs font-semibold text-zinc-600">Đối tượng nhận</p>
            <div>
              <p className="text-xs text-zinc-500 mb-1.5">Trạng thái lead</p>
            ...
            <div className="flex items-center gap-3 pt-1">
              ...
              {previewCount !== null && (
                <span className="text-xs text-zinc-600 font-medium">
                  ~<span className="text-zinc-900 font-bold">{previewCount}</span> người sẽ nhận
                </span>
              )}
            </div>
          </div>
      ...
      <div ref={overlayRef} className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 backdrop-blur-sm p-4 overflow-y-auto"
        ...
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl my-8">
          ...
          <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-zinc-600 mb-1">Tên template *</label>
                <input className={inputCls} value={form.name} onChange={set('name')} required placeholder="Email chào mừng" />
              </div>
              <div>
                <label className="block text-xs font-medium text-zinc-600 mb-1">Tiêu đề email (Subject)</label>
                <input className={inputCls} value={form.subject} onChange={set('subject')} placeholder="Chào mừng bạn đến với..." />
              </div>
            </div>
            <div>
              <label className="block text-xs font-medium text-zinc-600 mb-1">Nội dung HTML</label>
              <textarea
                className={`${inputCls} font-mono text-xs resize-y`} rows={10}
                value={form.htmlBody} onChange={set('htmlBody')}
                placeholder="<html><body>...</body></html>"
              />
              <p className="text-xs text-zinc-400 mt-1">{form.htmlBody.length} ký tự</p>
            </div>
            <div>
              <label className="block text-xs font-medium text-zinc-600 mb-1">Nội dung văn bản thuần (tuỳ chọn)</label>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between px-5 py-3 border-b border-zinc-100 shrink-0">
              <p className="text-sm font-semibold text-zinc-900">Xem trước HTML</p>
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-xl border border-zinc-200 flex flex-col items-center justify-center py-16 text-zinc-400">
          ...
          <button onClick={() => setModalOpen(true)} className="mt-3 text-sm text-zinc-900 hover:underline">Tạo template đầu tiên</button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {templates.map(t => (
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-5">
        <div className="bg-white rounded-xl border border-zinc-200 p-4">
          ...
          <p className="text-sm text-zinc-500">Tổng chiến dịch</p>
        </div>
        <div className="bg-white rounded-xl border border-zinc-200 p-4">
          <p className="text-2xl font-bold text-green-600">{stats.active}</p>
          <p className="text-sm text-zinc-500">Đang chạy</p>
        </div>
        <div className="bg-white rounded-xl border border-zinc-200 p-4">
          <p className="text-2xl font-bold text-zinc-900">{stats.totalSent.toLocaleString()}</p>
          <p className="text-sm text-zinc-500">Tổng đã gửi</p>
        </div>
      </div>
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">Quản lý chiến dịch và templates marketing</p>
        </div>
      </div>
