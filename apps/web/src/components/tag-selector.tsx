  ...
  return (
    <div className="relative" ref={popoverRef}>
      ...
      <div className="flex flex-wrap gap-1.5 items-center">
        ...
        <button
          onClick={() => setOpen(o => !o)}
          className="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-xs text-zinc-400 border border-dashed border-gray-300 hover:border-zinc-400 hover:text-zinc-700 transition-colors"
        >
          <Plus size={10} />Tag
        </button>
      </div>
      ...
      {/* Popover */}
      {open && (
        <div className="absolute left-0 top-full mt-1.5 w-60 bg-white rounded-xl shadow-lg border border-zinc-200 z-50 overflow-hidden">
          {/* Search / Create input */}
          <div className="px-3 py-2 border-b border-zinc-100">
            <div className="flex items-center gap-2">
              <TagIcon size={13} className="text-zinc-400 shrink-0" />
              <input
                ref={inputRef}
                value={input}
          ...
          <div className="max-h-48 overflow-y-auto py-1">
            {isNew && (
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 text-xs hover:bg-zinc-50 text-zinc-900 transition-colors disabled:opacity-60"
              >
                <span
            ...
            {filtered.map(tag => (
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 text-xs hover:bg-zinc-50 transition-colors disabled:opacity-60"
              >
                <span
                  className="w-3.5 h-3.5 rounded-full shrink-0"
                ...
                {entityTagIds.has(tag.id) && <Check size={12} className="text-zinc-900 shrink-0" />}
              </button>
            ))}
          </div>
        </div>
